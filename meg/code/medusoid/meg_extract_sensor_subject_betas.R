...
sensors = F # this is just to inspect the data

if (sensors) {
# load all models
regressors = c("entropy_change_rs","v_max_rs")
...
all_ress <- lapply(regressors, function(rr) {
  ...
  ddf <- readRDS(dfile) %>% 
    ...
    filter((effect=="ran_vals" & group == "Subject") | (effect=="fixed" & term=="(Intercept)"))  %>% droplevels() %>% 
    dplyr::select(t, Freq, level, effect, term, estimate, std.error, conf.low, conf.high) %>% rename(id = level)
  return(ddf)
})

all_dfs <- data.table::rbindlist(all_ress) %>%  mutate(freq_lin = as.numeric(as.character(Freq)))

# inspect with respect to non-specific frequency effects
# first the intercepts
# the post-response intercept seems like the best approximation of overall frequency effect
idf <- all_dfs %>% filter(term=="(Intercept)" & effect == "fixed" & t < 0 & t > -0.3) %>% 
  select(freq_lin, t, estimate) %>% group_by(freq_lin) %>% summarise(mean_intercept = mean(estimate, na.rm = T))
ggplot(idf, aes(freq_lin, mean_intercept)) + geom_point()

all_dfs <- all_dfs %>% inner_join(idf, by = "freq_lin") %>% mutate(estimate_corrected = scale(estimate*-mean_intercept))
all_dfs <- all_dfs %>% filter(effect == "ran_vals" & term != "(Intercept)") 
ggplot(all_dfs, aes(estimate, estimate_corrected, color = freq_lin)) + geom_point() + facet_wrap(~term)

# setwd('~/Desktop/')
# pdf("meg_betas_inspection.pdf", height = 20, width = 30)
# ggplot(all_dfs , aes(freq_lin, estimate)) + geom_jitter() + facet_wrap(id~term)
# dev.off()
# pdf("meg_corrected_betas_inspection.pdf", height = 20, width = 30)
# ggplot(all_dfs , aes(freq_lin, estimate_corrected)) + geom_jitter() + facet_wrap(id~term)
# dev.off()
...
ec1 <- all_dfs %>% filter(term == "entropy_change_t" & t >= 0.4 & t <= 0.8 & Freq >= "8.4" &  Freq <= "16.8") %>% # & Freq < "16.8"
  group_by(id) %>%
  summarize(avg=mean(estimate), avg_corr=mean(estimate_corrected)) %>% mutate(reg_region = "entropy_change_late_beta")

ec2 <- all_dfs %>% filter(term == "entropy_change_t" & t >= -0.2 & t <= 0.1 & Freq >= "8.4" &  Freq <= "16.8") %>% # & Freq < "16.8"
  group_by(id) %>%
  summarize(avg=mean(estimate), avg_corr = mean(estimate_corrected)) %>% mutate(reg_region = "entropy_change_early_beta")


vm1 <- all_dfs %>% filter(term == "v_max_wi" & t >= 0.5 & t <= 0.9 & Freq >= "5.9" &  Freq <= "11.8") %>% # & Freq < "16.8"
  group_by(id) %>%
  summarize(avg=mean(estimate), avg_corr = mean(estimate_corrected)) %>% mutate(reg_region = "vmax_late_beta")

r1 <- all_dfs %>% filter(term == "reward_t" & t >= 0.15 & t <= 0.45 & Freq >= "5" &  Freq <= "8.4") %>% # & Freq < "16.8"
  group_by(id) %>%
  summarize(avg=mean(estimate), avg_corr = mean(estimate_corrected)) %>% mutate(reg_region = "reward_early_theta")
r2 <- all_dfs %>% filter(term == "reward_t" & t >= 0.35 & t <= 0.9 &  Freq <= "4.2") %>% # & Freq < "16.8"
  group_by(id) %>%
  summarize(avg=mean(estimate), avg_corr = mean(estimate_corrected)) %>% mutate(reg_region = "reward_late_delta")

betas <- rbind(ec1, ec2, vm1, r1, r2) 
# betas <- rbind(ec1, vm1, r1, r2) %>% rename(id = level)
wbetas <- betas %>% pivot_wider(names_from = reg_region, values_from = c(avg, avg_corr))
just_betas <- wbetas %>% select(is.numeric)
cormat <- psych::corr.test(just_betas, method = "pearson")
corrplot::corrplot(cormat$r, order = "hclust")
...
# get fMRI data
fmri_ec_betas <- readRDS("~/OneDrive/collected_letters/papers/meg/plots/wholebrain/betas/fmri_ec_betas.RDS") %>% 
  mutate(id = as.character(id)) %>% select(id, ppc_ec_beta, mt_ec_beta, pfc_ec_beta)
