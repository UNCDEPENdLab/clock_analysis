...
stat_summaries <- F # get mean statistics for every signal
reprocess_betas <- T # repeat extraction of individual betas
sensors = F # extract sensor random slopes
rewFunc = F
fmri = F # whether to merge and correlate with fMRI betas
plots = F # for MEG stat summaries only, plot
...
if (reprocess_betas) {
  ...
  all_regs_individual <- lapply(regressors, function(rr) {
  ...
  all_dfs_individual <- data.table::rbindlist(all_regs_individual)
  
  
  # extract entropy change "betas"
  # ecdf <- all_dfs_individual %>% filter(term=="entropy_change_t")
  # late beta
  if (rewFunc) {
    ec1 <- all_dfs_individual %>% filter(regressor == "ec_rewfunc" & term == "entropy_change_t" & t >= 0.5 & t <= 0.8 & Freq >= "8.4" &  Freq <= "16.8") %>% # & Freq < "16.8"
      group_by(level, regressor, rewFunc) %>%
      summarize(avg=mean(estimate)) %>% mutate(reg_region = "entropy_change_late_beta")
    # early beta (rebound)
    ec2 <- all_dfs_individual %>% filter(regressor == "ec_rewfunc" & term == "entropy_change_t" & t >= -0.2 & t <= 0.1 & Freq >= "8.4" &  Freq <= "16.8") %>% # & Freq < "16.8"
      group_by(level, regressor, rewFunc) %>%
      summarize(avg=mean(estimate)) %>% mutate(reg_region = "entropy_change_early_beta")
    rtn <- all_dfs_individual %>% filter(regressor == "rt_next" & term == "rt_next" & t >= 0.5 & t <= 0.8 & Freq >= "8.4" &  Freq <= "16.8") %>%
      group_by(level, regressor, rewFunc) %>%
      summarize(avg=mean(estimate)) %>% mutate(reg_region = "rt_next_late_beta") 
    rr1 <- all_dfs_individual %>% filter(regressor == "reward_rewfunc" & term == "reward_t" & t >= 0.2 & t <= 0.4 & Freq >= "5" &  Freq <= "8.4") %>% # & Freq < "16.8"
      group_by(level, regressor, rewFunc) %>%
    ...
    ec1 <- all_dfs_individual %>% filter(regressor == "entropy_change" & term == "entropy_change_t" & t >= 0.5 & t <= 0.8 & Freq >= "8.4" &  Freq <= "16.8") %>% # & Freq < "16.8"
      group_by(level, regressor) %>%
      summarize(avg=mean(estimate)) %>% mutate(reg_region = "entropy_change_late_beta")
    # early beta (rebound)
    ec2 <- all_dfs_individual %>% filter(regressor == "entropy_change" & term == "entropy_change_t" & t >= -0.2 & t <= 0.1 & Freq >= "8.4" &  Freq <= "16.8") %>% # & Freq < "16.8"
      group_by(level, regressor) %>%
      summarize(avg=mean(estimate)) %>% mutate(reg_region = "entropy_change_early_beta")
    
    
    # ec1_20 <- all_dfs_individual %>% filter(regressor == "entropy_change" & term == "entropy_change_t" & t >= 0.5 & t <= 0.8 & Freq >= "8.4" &  Freq <= "20") %>% # & Freq < "16.8"
    #   group_by(level, regressor) %>%
    #   summarize(avg=mean(estimate)) %>% mutate(reg_region = "entropy_change_late_beta_20hz")
    # 
    # ec2_20 <- all_dfs_individual %>% filter(regressor == "entropy_change" & term == "entropy_change_t" & t >= -0.2 & t <= 0.1 & Freq >= "8.4" &  Freq <= "20") %>% # & Freq < "16.8"
    #   group_by(level, regressor) %>%
    ...
    vm1 <- all_dfs_individual %>% filter(regressor == "v_max" & term == "v_max_wi" & t >= 0.5 & t <= 0.9 & Freq >= "8.4" &  Freq <= "16.8") %>% # & Freq < "16.8"
      group_by(level) %>%
      summarize(avg=mean(estimate)) %>% mutate(reg_region = "vmax_late_beta")
    
    r1 <- all_dfs_individual %>% filter(regressor == "reward" & term == "reward_t" & t >= 0.2 & t <= 0.4 & Freq >= "5" &  Freq <= "8.4") %>% # & Freq < "16.8"
      group_by(level) %>%
      summarize(avg=mean(estimate)) %>% mutate(reg_region = "reward_early_theta")
    r2 <- all_dfs_individual %>% filter(regressor == "reward" & term == "reward_t" & t >= 0.4 & t <= 0.8 &  Freq <= "4.2") %>% # & Freq < "16.8"
      group_by(level) %>% 
      summarize(avg=mean(estimate)) %>% mutate(reg_region = "reward_late_delta")
    
    pe1 <- all_dfs_individual %>% filter(term == "pe_max" & regressor == "signed_pe_rs" & t >= 0.2 & t <= 0.4 & Freq >= "5" &  Freq <= "8.4") %>% # & Freq < "16.8"
      group_by(level) %>%
      summarize(avg=mean(estimate)) %>% mutate(reg_region = "pe_early_theta")
    
    abspe1 <- all_dfs_individual %>% filter(term == "abs_pe" & regressor == "abs_pe" & t >= 0.2 & t <= 0.4 & Freq >= "5" &  Freq <= "8.4") %>% # & Freq < "16.8"
      group_by(level) %>%
      summarize(avg=mean(estimate)) %>% mutate(reg_region = "abspe_early_theta")
    
    abspe2 <- all_dfs_individual %>% filter(term == "abs_pe" & regressor == "abs_pe" & t >= -0.2 & t <= 0.1 & Freq >= "8.4" &  Freq <= "16.8") %>% # & Freq < "16.8"
      group_by(level) %>%
      summarize(avg=mean(estimate)) %>% mutate(reg_region = "abspe_late_beta")
    
    pe2 <- all_dfs_individual %>% filter( term == "pe_max" & regressor == "signed_pe_rs" & t >= 0.5 & t <= 0.8 &  Freq >= "8.4" &  Freq <= "20") %>% # & Freq < "16.8"
      group_by(level) %>%
  ...
  if (rewFunc) {
    ...
    etheta <- wbetas %>% select(id, reward_early_theta, abspe_early_theta) %>% pivot_longer(names_to = "signal", 
                                                                                            cols =c(reward_early_theta, abspe_early_theta))
...
if (stat_summaries) {
  ...
  sec1 <- stat_df %>% filter(regressor == "entropy_change" & term == "entropy_change_t" & t >= 0.5 & t <= 0.8 & Freq >= "8.4" &  Freq <= "16.8") %>% # & Freq < "16.8"
    summarize(stat=mean(statistic),
              se = plotrix::std.error(statistic)) %>% mutate(reg_region = "entropy_change_late_beta")
  # early beta (rebound)
  sec2 <- stat_df %>% filter(regressor == "entropy_change" & term == "entropy_change_t" & t >= -0.2 & t <= 0.1 & Freq >= "8.4" &  Freq <= "16.8") %>% # & Freq < "16.8"
    summarize(stat=mean(statistic), se = plotrix::std.error(statistic)) %>% mutate(reg_region = "entropy_change_early_beta")
  ...
  #   summarize(stat=mean(statistic), se = plotrix::std.error(statistic)) %>% mutate(reg_region = "entropy_change_late_beta_20hz")
  # 
  # sec2_20 <- stat_df %>% filter(term == "entropy_change_t" & t >= -0.2 & t <= 0.1 & Freq >= "8.4" &  Freq <= "20") %>% # & Freq < "16.8"
  #   summarize(stat=mean(statistic), se = plotrix::std.error(statistic)) %>% mutate(reg_region = "entropy_change_early_beta_20hz")
  # 
  # svm1 <- stat_df %>% filter(term == "v_max_wi" & t >= 0.5 & t <= 0.9 & Freq >= "8.4" &  Freq <= "16.8") %>% # & Freq < "16.8"
  #   summarize(stat=mean(statistic), se = plotrix::std.error(statistic)) %>% mutate(reg_region = "vmax_late_beta")
  # 
  library(plotrix)
  sr1 <- stat_df %>% filter(term == "reward_t" & regressor == "reward" & t >= 0.2 & t <= 0.4 & Freq >= "5" &  Freq <= "8.4") %>% # & Freq < "16.8"
    summarize(stat=mean(statistic), se = plotrix::std.error(statistic)) %>% mutate(reg_region = "reward_early_theta")
  ...
  sr2 <- stat_df %>% filter(term == "reward_t" & t >= 0.4 & t <= 0.8 &  Freq <= "4.2") %>% # & Freq < "16.8"
    summarize(stat=mean(statistic), se = plotrix::std.error(statistic)) %>% mutate(reg_region = "reward_late_delta")
  
  sr3 <- stat_df %>% filter(term == "reward_t" & regressor == "reward" & t >= 0.5 & t <= 0.8 &  Freq >= "8.4" &  Freq <= "16.8") %>% # & Freq < "16.8"
    summarize(stat=mean(statistic), se = plotrix::std.error(statistic)) %>% mutate(reg_region = "reward_late_beta")
  
  spe1 <- stat_df %>% filter(term == "pe_max" & regressor == "signed_pe_rs" & t >= 0.2 & t <= 0.4 & Freq >= "5" &  Freq <= "8.4") %>% # & Freq < "16.8"
    summarize(stat=mean(statistic), se = plotrix::std.error(statistic)) %>% mutate(reg_region = "pe_early_theta")
  sabspe1 <- stat_df %>% filter(term == "abs_pe" & regressor == "abs_pe" & t >= 0.2 & t <= 0.4 & Freq >= "5" &  Freq <= "8.4") %>% # & Freq < "16.8"
    summarize(stat=mean(statistic), se = plotrix::std.error(statistic)) %>% mutate(reg_region = "abspe_early_theta")
  sabspe2 <- stat_df %>% filter(term == "abs_pe" & regressor == "abs_pe" & t >= 0.5 & t <= 0.8 &  Freq >= "8.4" &  Freq <= "16.8") %>% # & Freq < "16.8"
    summarize(stat=mean(statistic), se = plotrix::std.error(statistic)) %>% mutate(reg_region = "abspe_late_beta")
  ...
  spe2 <- stat_df %>% filter(term == "pe_max" & regressor == "signed_pe_rs" & t >= 0.5 & t <= 0.8 &  Freq >= "8.4" &  Freq <= "16.8") %>% # & Freq < "16.8"
    summarize(stat=mean(statistic), se = plotrix::std.error(statistic)) %>% mutate(reg_region = "pe_late_beta")
  ...
  if (plots) {
    ...
    beta_stats <- rbind(sec1, sabspe2, sr3)
