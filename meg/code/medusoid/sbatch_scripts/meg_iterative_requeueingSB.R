...
for (ee in epochs) {
  ...
  for (rr in regressors_of_interest) {
    ...
    out_expect <- file.path(epochdir, paste0("meg_mixed_by_tf_ddf_wholebrain_entropy_kld_sub_rs_single_", rr, "_rs_single_sensor_", ee, fnum))

    compute_expect <- file.path(epochdir, paste0(".", rr, "_it", fnum, "_SBcompute"))
      ...
      for (ff in seq_along(to_run)) {
        ...
        if (!test) {
          ...
          paste0(
            ...
            " sbatch_meg_mixed_wholebrain_ll_singleSB.bash"
          )
        )
