...
fef_censors <- c(612,543,1022)
ips_censors <- c(1823, 1822, 2222,2223)
all_censors <- c(fef_censors,ips_censors)
...
# } else { 
  ...
  colnames(rt) <- colnames(as_tibble(readRDS(paste0("MEG", ips_censors[1], "_20Hz.rds"))))
  for (censor in ips_censors) {
  c <- as_tibble(readRDS(paste0("MEG", censor, "_20Hz.rds")))
  c$censor <- as.character(censor)
  rt <- rbind(rt, c)
  }
  rt <- rt %>% rename(id = Subject, trial = Trial, run = Run, evt_time = Time)
# }
  ...
  # # 
  # if (!replicate_compression) {
    rt_comb <- trial_df %>% select(id, run, run_trial, score_csv,  rt_lag, rewFunc,
