...
# ggplot(esm1, aes(x = rewFunc, y=emmean, ymin=asymp.LCL, ymax=asymp.UCL, color=as.factor(omission_early_theta))) +
#   geom_point(position = position_dodge(width = .6), size=2.5) + 
...
# esm2 <- as_tibble(emmeans(sm1, data = df, ~rewFunc|omission_early_theta, at = list(omission_early_theta = c(-.04, .61))))
# ggplot(esm2, aes(x = rewFunc, y=emmean, ymin=asymp.LCL, ymax=asymp.UCL, color=as.factor(omission_early_theta))) +
#   geom_point(position = position_dodge(width = .6), size=2.5) +
