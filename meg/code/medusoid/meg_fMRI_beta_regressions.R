...
# basic model
m1 <- lmer(value ~ rewFunc * network * omission_early_theta + (1|id), df)
...
em1 <- as_tibble(emmeans(m1, data = df, ~network|omission_early_theta*rewFunc , at = list(omission_early_theta = c(-.04, .61))))
...
pe_betas <- read_csv("Schaefer2018_200Parcels_7Networks_order_fonov_2.3mm_ants_cope_l1.csv.gz") %>% filter(l1_cope_name=="EV_pe") %>% 
  dplyr::select(id, run_number, l1_model, mask_value, value) %>% mutate(id = as.character(id),
                                                                        ...
                                                                        run_mc  = scale(run_number, center = T, scale = F))
# merge
df <- pe_betas  %>% inner_join(wbetas, by = "id") %>% inner_join(design, by = c("id", "run_number")) %>% inner_join(labels, by = "mask_value")
...
# PE beta prediction
m1a <- lmer(value ~ rewFunc * network * omission_early_theta + (1|id), df)
...
summary(m1a)
Anova(m1a, '3')
em1a <-  as_tibble(emmeans(m1a, data = df, ~network|omission_early_theta*rewFunc , at = list(omission_early_theta = c(-.04, .61))))
...
m2 <- lmer(value ~ rewFunc * network * pe_late_beta_supp + (1|id), df)
...
em2 <- as_tibble(emmeans(m2, data = df, ~network|pe_late_beta_supp*rewFunc , at = list(pe_late_beta_supp = c(-1.750e-03, 1.057e-02))))
...
m3 <- lmer(value ~ rewFunc * network * abspe_early_theta + (1|id), df)
...
# .90        .95 
# 0.0061380  0.0066809 
em3 <- as_tibble(emmeans(m3, data = df, ~network|abspe_early_theta*rewFunc , at = list(abspe_early_theta = c(-0.0022299, .0078820061))))
...
m4 <- lmer(value ~ rewFunc * network * abspe_early_theta + rewFunc * network * omission_early_theta + (1|id), df)
...
Anova(m4, '3')
vif(m4)
em4 <- as_tibble(emmeans(m4, data = df, ~network|omission_early_theta*rewFunc , at = list(omission_early_theta = c(-.04, .61))))
...
em4a <- as_tibble(emmeans(m4, data = df, ~network|abspe_early_theta*rewFunc , at = list(abspe_early_theta = c(-0.0022299, .0078820061))))
ggplot(em4a, aes(x=network, y=emmean, ymin=asymp.LCL, ymax=asymp.UCL, color=as.factor(abspe_early_theta))) +
  geom_point(position = position_dodge(width = .6), size=2.5) +
  geom_errorbar(position = position_dodge(width=0.6), width=0.4, size=0.9) + facet_wrap(~rewFunc) +
  theme_bw(base_size=12) +  ylab("BOLD response to reward prediction error")  +
  scale_color_manual("Absolute PE\nearly theta\nresponse", values=c("#1b3840","#4fa3b8"), labels = c("10th %ile", "90th %ile")) +
...
m5 <- lmer(value ~ rewFunc * network * entropy_change_late_beta_supp + (1|id), df)
...
summary(m5)
Anova(m5, '3')
em5 <- as_tibble(emmeans(m5, data = df, ~network|entropy_change_late_beta_supp*rewFunc , at = list(entropy_change_late_beta_supp = c(-0.07553, 0.22084))))
...
ec_betas <- read_csv("Schaefer2018_200Parcels_7Networks_order_fonov_2.3mm_ants_cope_l1.csv.gz") %>% filter(l1_cope_name=="EV_entropy_change_feedback") %>% 
  dplyr::select(id, run_number, l1_model, mask_value, value) %>% mutate(id = as.character(id),
                                                                        ...
                                                                        run_mc  = scale(run_number, center = T, scale = F))
# merge
df <- ec_betas  %>% inner_join(wbetas, by = "id") %>% inner_join(design, by = c("id", "run_number"))  %>% inner_join(labels, by = "mask_value")

# not much for late beta outside of DMN
m6 <- lmer(value ~ rewFunc *  network * entropy_change_late_beta_supp + (1|id), df)
...
m6a <- lmer(value ~ rewFunc * network * entropy_change_early_beta_supp + (1|id), df)
...
em6 <- as_tibble(emmeans(m6, data = df, ~network|entropy_change_late_beta_supp*rewFunc , at = list(entropy_change_late_beta_supp = c(-0.07553, 0.22084))))
...
e_betas <- read_csv("Schaefer2018_200Parcels_7Networks_order_fonov_2.3mm_ants_cope_l1.csv.gz") %>% filter(l1_cope_name=="EV_entropy_wiz_clock") %>% 
  dplyr::select(id, run_number, l1_model, mask_value, value) %>% mutate(id = as.character(id),
                                                                        ...
                                                                        run_mc  = scale(run_number, center = T, scale = F))
# merge
df <- e_betas  %>% inner_join(wbetas, by = "id") %>% inner_join(design, by = c("id", "run_number"))  %>% inner_join(labels, by = "mask_value")

# late beta: again, mostly DMN
m7 <- lmer(value ~ rewFunc * network * entropy_change_late_beta_supp + (1|id), df)
...
summary(m7)
Anova(m7, '3')
em7 <- as_tibble(emmeans(m7, data = df, ~network|entropy_change_late_beta_supp*rewFunc , at = list(entropy_change_late_beta_supp = c(-0.07553, 0.22084))))
e_ec_lbeta <- ggplot(em7, aes(x=network, y=emmean, ymin=asymp.LCL, ymax=asymp.UCL, color=as.factor(entropy_change_late_beta_supp))) +
  geom_point(position = position_dodge(width = .6), size=2.5) +
  geom_errorbar(position = position_dodge(width=0.6), width=0.4, size=0.9) + facet_wrap(~rewFunc) +
...
pdf("MEG_to_fMRI_runwise_betas_rewFunc.pdf", height = 10, width = 16)
ggarrange(rewom_rewom_theta, pe_rewom_theta, pe_pe_beta, pe_ec_lbeta, ec_ec_lbeta, e_ec_lbeta, ncol = 2, nrow = 3)
dev.off()
