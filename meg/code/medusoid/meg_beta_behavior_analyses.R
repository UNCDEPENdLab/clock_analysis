...
mb_meg2 <-  lme4::lmer(rt_csv_sc ~ (trial_neg_inv_sc + rt_lag_sc + rt_vmax_lag_sc + last_outcome + 
                         ...
                         rt_lag_sc:last_outcome:vmax_late_beta +
                         rt_lag_sc:last_outcome:omission_early_theta +
                         rt_lag_sc:v_entropy_wi:entropy_change_late_beta + 
...
mb_meg3 <-  lme4::lmer(rt_csv_sc ~ (scale(run_trial) + rt_lag_sc + rt_vmax_lag_sc + last_outcome + 
                         ...
                         rt_lag_sc:last_outcome:vmax_late_beta +
                         rt_lag_sc:last_outcome:omission_early_theta +
                         rt_vmax_lag_sc:scale(run_trial):entropy_change_late_beta + 
...
# late beta to entropy change
em1 <- as_tibble(emtrends(mb_meg1_rs, data = df,  var = "rt_lag_sc", specs = c("entropy_change_late_beta", "last_outcome"), at = list(entropy_change_late_beta = c(-.14, .12)), options = list()))
em1$study = 'MEG'
em2 <- as_tibble(emtrends(fmb_meg1_rs, data = fdf, var = "rt_lag_sc", specs = c("entropy_change_late_beta", "last_outcome"), at = list(entropy_change_late_beta = c(-.14, .12)), options = list()))
em2$study = 'fMRI replication'
em1 <- rbind(em1, em2)
p1 <- ggplot(em1, aes(x=last_outcome, y=rt_lag_sc.trend, ymin=asymp.LCL, ymax=asymp.UCL, color=as.factor(entropy_change_late_beta))) + 
  #shape = as.factor(pe_f2_hipp), 
