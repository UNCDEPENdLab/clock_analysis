...
ggplot(wbetas, (aes(rewFunc, omission_early_theta, color = rewFunc))) + 
  geom_violin(draw_quantiles = .5) + geom_jitter(alpha = .3)
dev.off()
summary(lm(omission_early_theta ~ rewFunc, wbetas))

ggplot(wbetas, aes(rewFunc, rt_shorten_late_beta_supp, color = rewFunc)) + 
  geom_violin(draw_quantiles = .5) + geom_jitter(alpha = .3)
summary(lm(rt_shorten_late_beta_supp ~ rewFunc, wbetas))
# intercept NS, IEV<CEV,CEVR,DEV NS

pdf("ec_late_beta_by_cond.pdf", height = 3, width = 5)
ggplot(wbetas, aes(rewFunc, entropy_change_late_beta_supp, color = rewFunc)) + 
  geom_violin(draw_quantiles = .5) + geom_jitter(alpha = .3)
dev.off()
summary(lmer(entropy_change_late_beta_supp ~ rewFunc + (1|id), wbetas))

# merge
df <- df %>% inner_join(wbetas, by = c("id", "rewFunc"))
...
fdf <- fdf %>% mutate(id = as.character(id)) %>% inner_join(wbetas, by = c("id", "rewFunc"))
...
em1 <- as_tibble(emtrends(meg_late_beta_rewFunc, data = df,  var = "rt_lag_sc", specs = c("entropy_change_late_beta_supp", "last_outcome", "rewFunc"), at = list(entropy_change_late_beta_supp = c(-0.183, 0.317)), options = list()))
em1$study = "1. MEG"
em2 <- as_tibble(emtrends(fmri_late_beta_rewFunc, data = fdf, var = "rt_lag_sc", specs = c("entropy_change_late_beta_supp", "last_outcome", "rewFunc"), at = list(entropy_change_late_beta_supp = c(-0.183, 0.317)), options = list()))
em2$study = '2. fMRI replication'
em1 <- rbind(em2, em1)
ec_late_beta <- 
  ggplot(em1, aes(x=last_outcome, y=rt_lag_sc.trend, ymin=asymp.LCL, ymax=asymp.UCL, color=as.factor(entropy_change_late_beta_supp))) + 
...
emr1 <- as_tibble(emtrends(meg_all_late_beta_rewFunc, data = df,  var = "rt_lag_sc", specs = c("entropy_change_late_beta_supp", "last_outcome", "rewFunc"), at = list(entropy_change_late_beta_supp = c(-0.183, 0.317)), options = list()))
emr1$study = "1. MEG"
emr2 <- as_tibble(emtrends(fmri_all_late_beta_rewFunc, data = fdf, var = "rt_lag_sc", specs = c("entropy_change_late_beta_supp", "last_outcome", "rewFunc"), at = list(entropy_change_late_beta_supp = c(-0.183, 0.317)), options = list()))
...
oem1 <- as_tibble(emtrends(meg_om_theta_lbeta_rewFunc, data = df,  var = "rt_lag_sc", specs = c("omission_early_theta", "last_outcome", "rewFunc"), at = list(omission_early_theta = c(-0.120, 0.661)), options = list()))
oem1$study = "1. MEG"
oem2 <- as_tibble(emtrends(fmri_om_theta_lbeta_rewFunc, data = fdf, var = "rt_lag_sc", specs = c("omission_early_theta", "last_outcome", "rewFunc"), at = list(omission_early_theta = c(-0.120, 0.661)), options = list()))
oem2$study = '2. fMRI replication'
oem1 <- rbind(oem2, oem1)
omission_early_theta_rewFunc <- ggplot(oem1, aes(x=last_outcome, y=rt_lag_sc.trend, ymin=asymp.LCL, ymax=asymp.UCL, color=as.factor(omission_early_theta))) + 
...
pdf("meg_betas_to_behavior_rewFunc.pdf", height = 6, width = 15)
print(ggarrange(omission_early_theta_rewFunc, ec_late_beta, rt_late_beta, nrow = 1, ncol = 3))
dev.off()
