...
if (reprocess) {
  ...
  wddf <- pivot_wider(ddf %>% select(Time, t, Freq, AIC, BIC, model, alignment), names_from = model, names_sep = "_", values_from = c(AIC, BIC)) %>%
    mutate(AIC_diff = AIC_full - AIC_selective,
           AIC_log_diff = log(AIC_diff),
           BIC_diff = BIC_full - BIC_selective,
           BIC_log_diff = log(BIC_diff))
  saveRDS(wddf, file = paste0("meg_ddf_wholebrain_compression_", regressor, ".rds"))
  
  # saveRDS(rddf, file = "meg_ddf_wholebrain_ec_rs_rt.rds")
  message("Processed output, plotting.  \n")
  
  p1 <- ggplot(wddf %>% filter(alignment=="clock" & abs(AIC_diff) > 100), aes(Time, Freq)) + geom_tile(aes(fill = AIC_diff)) + 
    scale_fill_viridis(option = "plasma") + theme_dark()
  p2 <- ggplot(wddf %>% filter(alignment=="rt" & abs(AIC_diff) > 100), aes(Time, Freq)) + geom_tile(aes(fill = AIC_diff)) + 
    scale_fill_viridis(option = "plasma") + theme_dark()
  ggarrange(p2, p1)
  fname = paste0("meg_tf_compression_", regressor, ".pdf")
  pdf(fname, width = 10, height = 5)
  ggplot(wddf %>% filter(abs(AIC_diff) > 100), aes(t, Freq)) + geom_tile(aes(fill = AIC_diff)) + 
  ...
  if (!noclock) {ddf <- rbind(cddf, rddf)} else {
    ...
    wddf <- pivot_wider(ddf %>% select(Time, t, Freq, AIC, BIC, model, alignment), names_from = model, names_sep = "_", values_from = c(AIC, BIC)) %>%
      mutate(AIC_diff = AIC_full - AIC_selective,
             ...
             BIC_diff = BIC_full - BIC_selective,
             BIC_log_diff = log(BIC_diff))
    saveRDS(wddf, file = paste0("meg_ddf_wholebrain_compression_RT_", regressor, ".rds"))
  }
