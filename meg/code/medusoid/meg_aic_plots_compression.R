...
if (plots) {
  ...
  dev.off()
    ...
    ggplot(wddf , aes(t, Freq)) + geom_tile(aes(fill = AIC_diff_thresholded, alpha = !is.na(AIC_diff_thresholded))) + 
      ...
      geom_text(data = wddf, x = -0.4, y = 3.5,aes(label = "Response(t)"), size = 4, color = "black", angle = 90) +
      ...
      labs(fill = expression("AIC"["full"] - AIC["selective"])) + guides(alpha = F) +#scale_alpha(guide = 'none') + 
