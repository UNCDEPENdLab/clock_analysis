...
zero_rts <- tdf %>% filter(rt_csv==0) %>% 
  select(id, run, trial, rewFunc, emotion, rt_csv, score_csv, magnitude, probability, ev) %>%
...
Conclusion: there is something funky going on for some RTs where the backcalculation evaluates to > 4000, but the RT in the CSV file is substantially less.