...
if (unsmoothed) {
  ...
  setwd('~/Box/skinner/data/MRI/clock_bsocial//')
  meta <- read_csv("../clock_explore/v_entropy_z_index.csv")
  load("bsocial_extracted_roivalue.rdata")
}
...
if (unsmoothed) {
  ...
  pdf("bs_cluster_corr_fixed.pdf", width=24, height=24)
}
...
save(betas, file = "~/Box/skinner/data/MRI/clock_bsocial/bsocial_betas.rdata")
save(allbetas, file = "~/Box/skinner/data/MRI/clock_bsocial/bsocial_allbetas.rdata")
...
######
# PE cluster correlations after winsorizing
pejust_rois <- allbetas %>% select(contains('pe_max_cluster'))
peclust_cor <- corr.test(pejust_rois,method = 'pearson', adjust = 'none')
# parametric correlations on winsorised betas
# clust_cor <- cor(just_rois_w,method = 'pearson')

pdf("bsocial_pe_cluster_corr_fixed.pdf", width=12, height=12)
