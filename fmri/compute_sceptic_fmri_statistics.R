...
m <- melt(sigtrials, varnames=c("basis", "trial", "subject")) %>% arrange(subject, trial, basis) %>% select(-trial)#rename(trial_abs=trial)
m$run <- rep(1:8, each=50*24) #1:8 numbering
m$trial <- rep(1:50, each=24) #1:50 numbering as elsewhere

sigrestruct <- acast(m, subject ~ run ~ basis ~ trial, value.var="value") #tada! 
...
ventropy_decay_matlab <- melt(newEntropy$decay.H, varnames=c("subject", "trial"), value.name="entropyH")
ventropy_fixedlrv_matlab <- melt(newEntropy$fixed.H, varnames=c("subject", "trial"), value.name="entropyFixed")

ventropy_decay_matlab <- ventropy_decay_matlab %>% arrange(subject, trial)
ventropy_decay_matlab$run <- rep(1:8, each=50)
ventropy_decay_matlab$trial <- 1:50

ventropy_fixedlrv_matlab <- ventropy_fixedlrv_matlab %>% arrange(subject, trial)
ventropy_fixedlrv_matlab$run <- rep(1:8, each=50)
ventropy_fixedlrv_matlab$trial <- 1:50

ventropy_decay_matlab_3d <- acast(ventropy_decay_matlab, subject ~ run ~ trial, value.var="entropyH")
ventropy_fixedlrv_matlab_3d <- acast(ventropy_fixedlrv_matlab, subject ~ run ~ trial, value.var="entropyFixed")
...
tofmri <- list(V=fit$V, PE=fit$PE, D=fit$D, U=sigrestruct)
fmriexpanded <- list()
...
um$subject <- factor(um$LunaID, levels=sort(unique(um$LunaID)), labels=1:76)
#dcast(LunaID ~ run + trial + timestep, value.name="U")
...
rtumax <- apply(fmriexpanded[["U"]], c(1,2,3), function(trial) {
      if (sd(trial) < 1) { #essentially no variability, so hard to say there's a max 
        return(NA) 
      } else {
...
dimnames(ventropy_decay_matlab_3d) <- dimnames(ventropy_fixedlrv_matlab_3d) <- dimnames(rtumax)

save(vmax, vauc, vsd, ventropy, vchosen, pemax, peauc, dauc, dsd, umax, rtumax, ventropy_decay_matlab_3d, ventropy_fixedlrv_matlab_3d, ids, file="fmri_sceptic_signals_24basis.RData")
...
edf <- melt(ventropy, varnames=c("LunaID", "run", "trial"), value.name="entropy")
rtvmaxdf <- melt(rtvmax, varnames=c("LunaID", "run", "trial"), value.name="rtvmax")
vchosendf <- melt(vchosen, varnames=c("LunaID", "run", "trial"), value.name="vchosen")
vmaxdf <- melt(vmax, varnames=c("LunaID", "run", "trial"), value.name="vmax")
rtumaxdf <- melt(rtumax, varnames=c("LunaID", "run", "trial"), value.name="rtumax")
umaxdf <- melt(umax, varnames=c("LunaID", "run", "trial"), value.name="umax")
pemaxdf <- melt(pemax, varnames=c("LunaID", "run", "trial"), value.name="pemax")

bdf <- merge(edf, rtvmaxdf, by=c("LunaID", "run", "trial"))
bdf <- merge(bdf, vchosendf, by=c("LunaID", "run", "trial"))
bdf <- merge(bdf, vmaxdf, by=c("LunaID", "run", "trial"))
bdf <- merge(bdf, rtumaxdf, by=c("LunaID", "run", "trial"))
bdf <- merge(bdf, umaxdf, by=c("LunaID", "run", "trial"))
bdf <- merge(bdf, pemaxdf, by=c("LunaID", "run", "trial"))
bdf <- merge(bdf, allData, by=c("LunaID", "run", "trial"))

str(bdf)

bdf <- bdf %>% arrange(LunaID, run, trial)

bdf$subject <- rep(1:76, each=400) #Jon says decayH sorted ascending by subject
bdf <- merge(bdf, ventropy_decay_matlab, by=c("subject", "run", "trial"))

bdf <- merge(bdf, ventropy_fixedlrv_matlab, by=c("subject", "run", "trial"))


#bdf <- bdf %>% group_by(LunaID, run) %>% arrange(LunaID, run, trial) %>% mutate(timesteplag = Hmisc::Lag(timestep)) #do({.$tlag = Hmisc::Lag(.$timestep); return(.) }) %>% ungroup() # %>% ungroup()
#bdf <- bdf %>% group_by(LunaID, run) %>% arrange(LunaID, run, trial) %>% mutate(timesteplag = c(NA, diff(timestep))) #do({.$tlag = Hmisc::Lag(.$timestep); return(.) }) %>% ungroup() # %>% ungroup()

#need to be explicit about mutate because it was picking up mutate from plyr, which doesn't work properly here
bdf <- bdf %>% group_by(LunaID, run) %>%
    dplyr::mutate(
    ...
    ) %>% ungroup() %>% arrange(LunaID, run, trial)
   

library(tidyr)
bdf_toplot <- bdf %>% select(subject, run, trial, rtvmax, rtumax, timestep) %>% gather(key=signal, value=value, rtvmax, rtumax, timestep)

subids <- 1:length(unique(bdf_toplot$subject))

pdf("signals.pdf", width=10, height=15)
for (s in subids) {
  g <- ggplot(filter(bdf_toplot, subject==s), aes(x=trial, y=value, color=signal, shape=signal)) + facet_wrap(~run, ncol=1) + geom_line() + geom_point(position=position_jitter(width=0.5)) + ggtitle(s)
  plot(g)
}
dev.off()
...
for (s in subids) {
  g <- ggplot(filter(um, subject==s), aes(x=trial, y=timestep, color=U, fill=U)) + facet_wrap(~run, ncol=1) +  geom_raster(interpolate = TRUE, alpha=0.7) + ggtitle(s) #geom_tile() 
  plot(g)
}
...
for (s in subids) {
  #color=signal, 
#  g <- ggplot(filter(bdf_toplot, subject==s), aes(x=trial, y=value, shape=signal)) + facet_wrap(~run, ncol=1) + geom_line(position=position_dodge(width=0.5)) + geom_point(position=position_dodge(width=0.5)) + ggtitle(s) +
#      theme_bw(base_size=15) + geom_raster(data=filter(um, subject==s), aes(x=trial, y=timestep, fill = U, color=U, shape=NULL), interpolate = TRUE) +
#      scale_x_continuous("Trial") + scale_y_continuous("Timestep") 
#  plot(g)
  g <- ggplot(data=filter(um, subject==s), aes(x=trial, y=timestep, fill = U, color=NULL, shape=NULL)) + theme_bw(base_size=15) + 
      geom_raster(interpolate = TRUE) + facet_wrap(~run, ncol=1) +
      geom_point(data=filter(bdf_toplot, subject==s), aes(x=trial, y=value, shape=signal, color=signal, fill=NULL), position=position_dodge(width=0.5), size=1.5) + #, fill=NULL
      geom_line(data=filter(bdf_toplot, subject==s), aes(x=trial, y=value, color=signal, fill=NULL), position=position_dodge(width=0.5), size=1.5) + #, fill=NULL #shape=signal,  
...
g <- ggplot() +  geom_raster(data=filter(um, subject==s), aes(x=trial, y=timestep, fill = U, color=U, shape=NULL), interpolate = TRUE) + facet_wrap(~run, ncol=1) + #+ geom_line(position=position_dodge(width=0.5)) + geom_point(position=position_dodge(width=0.5)) + ggtitle(s) +
    theme_bw(base_size=15) + geom_line(data)
plot(g)


#looks correct
bdf %>% select(LunaID, run, trial, rt, timestep, timesteplag, timestepchange, rtvmax, vdev, vdevlag, score, omission) %>% arrange(LunaID, run, trial) %>% print(n=101)

bdf %>% select(LunaID, run, trial, rt, timestep, score, omission, entropy, wizentropy) %>% arrange(LunaID, run, trial) %>% print(n=101)

#now fit basic model: timestep change is predicted by prior deviation from value max and entropy 
library(lme4)

#save(file="dataframe_for_entropy_analysis_Oct2016.RData", bdf)
