...
analysis_dir <- "~/Library/CloudStorage/OneDrive-UniversityofPittsburgh/Documents/SCEPTIC_fMRI/bsocial/bsocial_betas_oct2022"
setwd(analysis_dir)

source("../get_trial_data.R")
source("parcel_brain_behavior_functions.R")
source("~/code/dependlab/R/cor_with_target.R")
source("~/code/dependlab/R/cor_heatmap.R")

# location of whole brain betas for analysis
# beta_dir <- "/Volumes/GoogleDrive/My Drive/SCEPTIC_fMRI/wholebrain_betas"
beta_dir <- file.path(analysis_dir, "L1m-entropy")

# MNH/AD internal labeling scheme
parcel_lookup <- read_csv("~/Library/CloudStorage/OneDrive-UniversityofPittsburgh/Documents/SCEPTIC_fMRI/explore_wholebrain/Schaefer_444_region_lookup.csv") %>%
  select(roi_num, MNI_Glasser_HCP_v1.0) %>% mutate(MNI_Glasser = sub("Focus point:\\s+", "", MNI_Glasser_HCP_v1.0, perl=TRUE),
                                                   MNI_Glasser = sub("Within [0-9] mm:\\s+", "", MNI_Glasser, perl=TRUE)) %>% select(-MNI_Glasser_HCP_v1.0)
labels <- read_csv("~/Library/CloudStorage/OneDrive-UniversityofPittsburgh/Documents/SCEPTIC_fMRI/explore_wholebrain/Schaefer_444_region_labels.csv") %>%
  mutate(roi_num = parse_number(roi_num)) %>% merge(parcel_lookup, by = "roi_num") %>% mutate(
    parcel = case_when(
      !is.na(parse_number(substr(subregion,1,2))) ~ MNI_Glasser,
      T ~ subregion
    ),
    parcel = sub("L|R\\_", "", parcel), 
    parcel = paste0(network, "_", hemi, parcel)
  ) %>% select(roi_num, network, parcel)

networks <- unique(labels$network)[complete.cases(unique(labels$network))]

# import subject characteristics
sub_df <- read_csv("~/code/bsocial_clock/data/personality_FA.csv")
traits <- names(sub_df %>% select(-id, -`...1`))
pid5 <- traits[(str_detect(traits, "pid5"))]
big5 <- traits[str_length(traits)<7]
ipip_facets <- traits[str_length(traits)>7 & str_detect(traits, "ipip")]
trait_list <- list(pid5, big5, ipip_facets)
# import betas
efiles_l2 <- list.files(beta_dir,
                        pattern = "Schaefer_444_final_2009c_3.125mm_cope_l2.csv.gz",
                        recursive = TRUE, full.names = TRUE
)

betas <- read_csv(efiles_l2) %>% select(id, mask_value, value, l1_cope_name) %>% rename(roi_num = mask_value) %>% 
  filter(l1_cope_name == "EV_entropy_clock") %>% merge(labels, by = "roi_num")
wbetas <- betas %>% select(-l1_cope_name, -network) %>% pivot_wider(names_from = c(parcel, roi_num), values_from = value)
str(wbetas)

df <- merge(wbetas, sub_df, by = "id")
# pid5 = c("")
blocks <- c("pid5", "big5", "ipip_facets")
for (network in networks) {
  for (block in 1:length(trait_list)) {
    traits <- trait_list[[block]]
    cormat <- cor_with_target(df, target = traits, withvars = df %>% select(contains(network)) %>% names())
    
    pdf(paste0(network, "_", blocks[block], ".pdf"), height = 20, width = length(traits)*1.5)
    print(ggplot(cormat, aes(targetvar, withvar, fill = r)) + geom_raster() + scale_fill_gradient2(limits = c(-.4, .4), name="Correlation\n", low=muted("blue"), high=muted("red")) +
            geom_text(aes(targetvar, withvar, label = round(r, 2)), colour="grey30", show.legend=FALSE, size=tileTextSize, hjust=0.13, vjust=0.5, angle=0) +
            xlab("") + ylab(""))
    dev.off()
  }
}
# cor_heatmap(df, cormat$r, alphaSort = F)
# #efiles <- efiles[3:6]
# for (ee in efiles_l2) {
#   job <- R_batch_job$new(
