...
colorby <- "subregion" #options are subregion and structure
...
setwd(map_dir)
labels <- fread("Schaefer_445_region_labels.csv")
...
} else if (toprocess == "subregion") {plot_df <- ddf$coef_df_reml %>% inner_join(labels %>% select(subregion, structure, network) %>% unique(), by="subregion")
...
if (region_to_run == "subcortical") {if (split_hipp_amyg){
  color_scheme <- c('darkgoldenrod','#c912ab','brown4','darkgreen','#644bc8', 'darkred', 'darkslategrey')
