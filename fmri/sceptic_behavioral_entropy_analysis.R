...
bdf <- bdf %>% group_by(LunaID, run) %>% mutate(
    rtlag=lag(rt, order_by=run)
)

summary(m1 <- lmer(rt ~ rtlag + rtvmaxlag + rtumaxlag + (1|LunaID) + (1|run), bdf, REML=FALSE))
summary(m2 <- lmer(rt ~ rtlag + rtvmaxlag + rtumaxlag + (1|LunaID) + (1|LunaID:run), bdf, REML=FALSE))
summary(m3 <- lmer(rt ~ rtlag + rtvmaxlag + rtumaxlag + (1 |LunaID) + (1 + rtumaxlag|run) + (1|LunaID:run), bdf, REML=FALSE))
anova(m1, m2, m3)
