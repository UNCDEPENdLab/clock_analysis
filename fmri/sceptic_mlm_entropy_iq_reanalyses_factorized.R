...
setwd("~/Box Sync/skinner/projects_analyses/SCEPTIC/subject_fitting/")
...
fse <-read_csv("~/Box Sync/skinner/projects_analyses/SCEPTIC/fMRI_paper/signals_review/compiled_outputs/mmclock_fmri_decay_factorize_selective_psequate_mfx_trial_statistics.csv.gz")
fsn <- read_csv("~/Box Sync/skinner/projects_analyses/SCEPTIC/fMRI_paper/signals_review/compiled_outputs/mmclock_fmri_decay_factorize_selective_psnarrow_mfx_trial_statistics.csv.gz")
fun <- read_csv("~/Box Sync/skinner/projects_analyses/SCEPTIC/fMRI_paper/signals_review/compiled_outputs/mmclock_fmri_decay_factorize_uniform_psnarrow_mfx_trial_statistics.csv.gz")
sse <- read_csv("~/Box Sync/skinner/projects_analyses/SCEPTIC/fMRI_paper/signals_review/compiled_outputs/mmclock_fmri_decay_separate_selective_psnarrow_mfx_trial_statistics.csv.gz")

df1 <- inner_join(fse[,3:26],fsn[,3:26], by = names(fsn)[c(3:16)], suffix = c(".fse", ".fsn"))
df2 <- inner_join(fun[,3:26],sse[,3:26], by = names(fun)[c(3:16)], suffix = c(".fun", ".sse"))
bdf <- inner_join(df1,df2)
# bdf <- bdf %>% rename(subject=rowID)
# bdf <- left_join(bdf, idlist, by="subject")
bdf$score <- bdf$score_csv
# change 'trial' to within-run trial; asc_trial remains
bdf$trial <- bdf$trial - (bdf$run - 1) * 50
#compute total rewards and cumulative rewards by subject
bdf = bdf %>% group_by(id) %>% arrange(id, run, trial) %>% mutate(totreward=sum(score), cumreward=cumsum(score)) %>% ungroup() %>%
...
bdf$ID <- bdf$id
# need to merge correctly
bdf <- bdf %>% arrange(ID,run,trial)

# ggplot(mbdf,aes(rt,rt_csv)) + geom_point() + facet_wrap(~id)

###
#plot average entropy for narrow decay (old) and selective wide decay (new) models
edescriptives <- bdf %>% 
  dplyr::select(id, run, trial, msplit, v_entropy.fse,v_entropy.fsn, v_entropy.fun, v_entropy.sse) %>% 
  gather(key=Model, value=entropy,v_entropy.fse,v_entropy.fsn, v_entropy.fun, v_entropy.sse) %>% 
  mutate(Model=recode(Model, v_entropy.fse = "FSE",v_entropy.fsn = "FSN", v_entropy.fun = "FUN", v_entropy.sse = "SSE"))


edesc_aggruns <- edescriptives %>% filter(run > 1) %>%
  group_by(id, Model, trial) %>% dplyr::summarize(entropy=mean(entropy))

ggplot(edescriptives, aes(x=trial, y=entropy, color=Model)) + 
...
edesc_aggtrialsruns_msplit <- edescriptives %>% filter(run > 1) %>% 
...
pdf("fse fsn fun sse entropy curves raw means.pdf", width=5, height=3.5)
...
pdf("fse fsn fun sse entropy curves raw means inset.pdf", width=3.5, height=2.5)
#ggplot(filter(edesc_aggtrialsruns), aes(x=trial, y=m, ymin=m-se, ymax=m+se, color=Model)) +
ggplot(edesc_aggtrialsruns, aes(x=trial, y=Mean, ymin=Lower, ymax=Upper, color=Model)) +
...
pdf("fse fsn fun sse entropy curves raw means inset msplit.pdf", width=7.9, height=2.5)
#ggplot(filter(edesc_aggtrialsruns), aes(x=trial, y=m, ymin=m-se, ymax=m+se, color=Model)) +
