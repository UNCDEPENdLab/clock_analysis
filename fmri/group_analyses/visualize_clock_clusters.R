...
fslgroupdir <- "/Volumes/Serena/MMClock/fsl_group"

#cope 6 is relative uncertainty in LVL1 runs
l2dirs <- data.frame(dir=dirname(readLines(file.path(fslgroupdir, "cope6inputs")))) #use dirname to pull off cope6
...
setwd("/Users/michael/Data_Analysis/clock_analysis/fmri/group_analyses/cluster_plots_fsl_Jun2015")
...
l1copes <- c("clock_onset", "feedback_onset", "ev", "rpe_pos", "rpe_neg", "rel_unc")
l2copes <- c("m_scram", "m_fear", "m_happy", "m_overall", "fear_gt_scram", "happy_gt_scram", "fear_gt_happy")
l3copes <- c("intercept", "female", "age", "explorer", "age_explorer")
  ...
  for (l2 in 1:length(l2copes)) {
    ...
    for (l3 in 1:length(l3copes)) {
      ...
      runAFNICommand(paste0("3dclust -overwrite -1Dformat -nosum -1dindex 0 -1tindex 0",
              " -1thresh ", zthresh, " -dxyz=1 -savemask tmpclust 1.01 40 ", groupmap), 
          afnidir=afnidir, stdout="tmpclust.1D") #/Volumes/Serena/bars_ica/output/einfomax_60_30_prenorm_28Jan2014/ic11behavLMER+tlrc.HEAD
      ...
      for (k in 1:length(bestguess)) {
        ...
        if (l3copes[l3] == "age_explorer") {
          ...
          plotnote <- paste0("b = ", round(int_b, 3), ", t = ", round(int_t, 3), ", p = ", round(int_p, 4))
          
          g <- ggplot(df, aes(x=age, y=vox, color=explorer)) + geom_point() + stat_smooth(method="lm") +
              ggtitle(plottitles[k]) + annotate("text", x = min(df$age), y = max(df$vox), label = plotnote, hjust=0)
          plot(g)
        } else if (l3copes[l3] == "age") {            
          ...
          plot(g)
        } else if (l3copes[l3] == "explorer") {
          m <- t.test(vox ~ explorer, df)
          plotnote <- paste0("t(", round(m$parameter, 2), ") = ", round(m$statistic, 2), ", p = ", round(m$p.value, 4))
          g <- ggplot(df, aes(x=explorer, y=vox)) + geom_boxplot() + 
              ggtitle(plottitles[k]) + annotate("text", x = 1, y = max(df$vox)+0.1*max(df$vox), label = plotnote, hjust=0)
          plot(g)          
...
save(graphlist, file="cluster_ggplot_objs_Jun2015.RData")


load(file="cluster_ggplot_objs_Jun2015.RData")
dimnames(graphlist)
