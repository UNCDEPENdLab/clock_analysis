...
model_clock_fmri_lvl1 <- function(trial_statistics, id_col=NULL, subject_covariates=NULL, drop_volumes=6, ncpus=1,forcererun=FALSE,
  ...
  if(!exists("forcererun")){forcererun<-FALSE}
  ...
  #midpoint<-length(by_subj_og)/2
  #by_subj<-by_subj_og[1:midpoint]
  #by_subj<-by_subj_og[(midpoint+1):length(by_subj_og)]
  ...
  NXU<-parallel::parLapply(cl, by_subj, function(b){
      ...
      #system(paste("echo",subid),intern = F)
      mrfiles <- c() #forcererun clear of mr files over subjects to avoid potential persistence from one subject to the next

      mrmatch <- subject_covariates$mr_dir[subject_covariates[[id_col]] == subid]

      if (length(mrmatch) != 1L) {
        system(paste("echo",paste("Unable to find fMRI directory for subid: ", subid)),intern = F)

        return(NULL)
      }

      if (! file.exists(file.path(mrmatch, expectdir))) {
        system(paste("echo",paste("Unable to find preprocessed data ", expectdir, " for subid: ", subid)),intern = F)
        return(NULL)
      }
      ...
      if (length(mrfiles) == 0L) {
        system(paste("echo",paste("Unable to find any preprocessed MB files in dir: ", mrmatch)),intern = F)
        return(NULL)
      }

      if(length(mrfiles)>0){
        #system(paste("echo",paste("X1",subid)),intern = F)
        fsl_run_output_dir <- file.path(normalizePath(file.path(dirname(mrfiles[1L]), "..")), outdir,"designmatrix.RData")

        #system(paste("echo",paste("X1.5",fsl_run_output_dir,subid)),intern = F)

        if (file.exists(fsl_run_output_dir) && forcererun==FALSE) {
          system(paste("echo",paste(fsl_run_output_dir, " exists. Skipping.")),intern = F); return(NULL) }
      }else {
        warning("No DATA")
        return(NULL)
      }
      #system(paste("echo",paste("X2",subid)),intern = F)
