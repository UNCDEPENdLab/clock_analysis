...
cache_dir = "~/Box/SCEPTIC_fMRI/vmPFC/cache"
...
source(file.path("~/vmPFC/load_medusa_data_vmPFC.R"))
...
d <- merge(df, fb_comb, by = c("id", "run", "run_trial"))
...
decode_formula = formula(~ trial_neg_inv_sc + rt_csv_sc + rt_lag_sc + scale(rt_vmax_lag)  + scale(rt_vmax_change) + 
                           v_entropy_wi + v_entropy_wi_change + kld3_lag  + v_max_wi  + scale(abs_pe) + outcome + (1|id))

source("/Users/andypapale/vmPFC/fmri.pipeline-master/R/mixed_by.R")
ddf <- mixed_by(d, outcomes = "decon_interp", rhs_model_formulae = decode_formula , split_on = splits,
                padjust_by = "term", padjust_method = "fdr", ncores = 1, refit_on_nonconvergence = 3)
...
decode_results_fname = "rt_decode_output_streams_mixed_by.Rdata"
ddf <- as_tibble(ddf)
ddf$t <- ddf$evt_time
...
for (fe in terms) {
  ...
  fname = paste("fb_network", termstr, "_mixed_by.pdf", sep = "")
  ...
  # fname = paste("region_line_", termstr, "_mixed_by.pdf", sep = "")
  # pdf(fname, width = 9, height = 3.5)
