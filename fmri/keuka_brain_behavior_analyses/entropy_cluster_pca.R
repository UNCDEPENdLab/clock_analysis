...
ggplot(df, aes(rt_lag, rt_csv, color = h_f1_fp>0, lty = h_f2_neg_paralimb>0)) + geom_smooth(method = "glm")+ facet_wrap(~run_trial>20)
ggplot(df, aes(rt_vmax, rt_csv, color = h_f1_fp>0, lty = h_f2_neg_paralimb>0)) + geom_smooth(method = "glm")+ facet_wrap(~run_trial>20)

pdf("h_timecourse_brain_fixed.pdf", width = 8, height = 8)
ggplot(df, aes(run_trial, v_entropy, color = h_f1_fp>0, lty = h_f2_neg_paralimb>0)) + geom_smooth(method = "loess") + facet_wrap(~gamma>0)
dev.off()
