...
m1 <-  lmer(rt_csv_sc ~ (trial_neg_inv_sc + rt_lag_sc + last_outcome)^2 + 
                       (1|id/run), df)
screen.lmerTest(m1, .05)
summary(m1)
Anova(m1, '3')

m2 <-  lmer(rt_csv_sc ~ (trial_neg_inv_sc + rt_lag_sc + rt_vmax_lag_sc + last_outcome + 
                           v_max_wi_lag + v_entropy_wi)^2 + 
              (1|id/run), df)
screen.lmerTest(m2, .05)
summary(m2)
Anova(m2, '3')
