...
if (meg) {
  ...
  ddf <- readRDS("~/Library/CloudStorage/OneDrive-UniversityofPittsburgh/Documents/SCEPTIC_fMRI/dan_medusa/rt_400_final47_encode_medusa_fmri.rds")
}

out_dir <- "~/Library/CloudStorage/OneDrive-UniversityofPittsburgh/Documents/SCEPTIC_fMRI/dan_medusa/"
...
ddf <- readRDS("~/Library/CloudStorage/OneDrive-UniversityofPittsburgh/Documents/SCEPTIC_fMRI/dan_medusa/rt_400_final47_encode_medusa_fmri_rslopeonly.rds")
...
pdf("~/Library/CloudStorage/OneDrive-Personal/collected_letters/papers/meg/figures/fig_3_echange_whole_brain_medusa/vmax_medusa.pdf", width=4.5, height=3)
ggplot(my_df, aes(x=evt_time, y=estimate, ymin=estimate-std.error, ymax=estimate + std.error, color=vm_gradient17, group=vm_gradient17, size=p_level)) +
  common +
  ylab("Vmax coefficient (AU)") + xlab("Time relative to feedback (seconds)") + theme(legend.position = "none")
dev.off()
...
dev.off() } else if (study == "explore") {

# Explore replication:

# load ddf# 
ddf <- readRDS("~/Library/CloudStorage/OneDrive-UniversityofPittsburgh/Documents/SCEPTIC_fMRI/explore_medusa/rt_parcel_group_rt_logkld3_rslope_bilateral_Sept_27_2022.rds")
get_df_explore <- function(ddf, model_name, term) {
  require(dplyr)
  ddf$coef_df_reml %>% filter(model_name==!!model_name & effect=="fixed" & abs(evt_time) <= 5) %>%
    filter(term == !!term) %>%
    mutate(..p=p.adjust(p.value, method="fdr")) %>%
    mutate(
      p_level = case_when(
    ...
    mutate(
      vm_gradient17 = ordered(parcel_group, levels=c("MT+", "PPCcaudal", "PPCrostral", "Premotor"), labels=c("MT+", "Caudal PPC", "Rostral PPC", "Premotor"))
    ) %>%
    filter(evt_time < 5) # only 0-4 post-feedback
}
# get random slope, Abs PE, logkld model

my_df <- get_df_explore(ddf, "rt_rslope_logkld3_bl", "v_entropy_wi_change")

# entropy_wi_change plot
pdf("~/Library/CloudStorage/OneDrive-Personal/collected_letters/papers/meg/figures/fig_3_echange_whole_brain_medusa/explore_entropy_wi_change_rslope_logkld.pdf", width=8, height=5)
ggplot(my_df, aes(x=evt_time, y=estimate, ymin=estimate-std.error, ymax=estimate + std.error, color=vm_gradient17, group=vm_gradient17, size=p_level)) +
  common +
  ylab("Entropy change coefficient (AU)") + xlab("Time relative to feedback (seconds)")
dev.off()
