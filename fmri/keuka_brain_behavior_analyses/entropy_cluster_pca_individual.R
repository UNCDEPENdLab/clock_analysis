...
# incorporate H
summary(mrh2 <- lmer(scale(rt_csv) ~ (scale(run_trial) + scale(rt_vmax) + scale(rt_lag) + scale(v_entropy) + h_pc1 + scale(vmPFC))^3 + rewFunc + (1|id/run), df))
summary(mrh3 <- lmer(scale(rt_csv) ~ (scale(run_trial) + scale(rt_vmax) + scale(rt_lag) + scale(v_entropy) + h_pc1 + scale(ParaHippL))^3 + rewFunc + (1|id/run), df))

summary(mrh4 <- lmer(scale(rt_csv) ~ (scale(run_trial) + scale(rt_vmax) + scale(rt_lag) + scale(v_entropy) + scale(h_f1) + scale(h_f2))^3 + rewFunc + (1|id/run), df))

# check H timecourses in high vs. low beta subjects
pdf("h_timecourse_brain_gamma_indiv.pdf", width = 12, height = 12)
ggplot(df, aes(run_trial, v_entropy, color = h_pc1>0, lty = h_pc2>0)) + 
  geom_smooth(method = "loess") + facet_wrap(~df$gamma>0)
dev.off()
# by condition
ggplot(df, aes(run_trial, v_entropy, color = h_pc1>0, lty = h_pc2>0)) + 
  geom_smooth(method = "loess") + facet_wrap(~df$rewFunc)
